...
int main(int argc, char *argv[]){
	...
	n = atoi(argv[1]);
	thread_id2=(pthread_t*)malloc(n*sizeof(pthread_t));
	for(i = 0; i < n; i++){
		rc2 = pthread_create(&thread_id2[i], NULL, doorMan, (void*)&n);
		if(rc2){
		      printf("\n ERROR: return code from pthread_create is %d \n", rc2);
		      exit(1);
	   }
	   else{
		      printf("\nCreate a new thread for doorMan\n");
	 }
	}
	...
	if(tooManyZombiesInTheRoom() == 1){
		printf("have been killed by the zombies!!\n");
	}
